...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{Workflow3Flowchart}
...
We tried to increase this feature count, but then realised, that it is not possible to set parameters for detectors in OpenCV 3.1. The problem is that the method, to read and write algorithm parameters, is implemented in the Java wrapper, but is missing in the C++ implementation (http://answers.opencv.org/question/76074/send-parameters-to-orb-feature-detector-android/).
...
Figure~\ref{fig:CCT2ExampleAN1} shows an example of how the second iteration was performing. Nearly every element on the floor plan is detected as door. This lead us to conclusion, that the door is trained with too few features, like the ORB method (Section~\ref{sub:ImpORB}).
...
As improvement, we tried to use the cascade classification together with an other method, which would classify the huge amount of positive images and filter out the false positives.

\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\textwidth]{CCT3PolyDPResult}
	\caption{Results of the third cascade classification experiment with polygon matching.}
...
\begin{figure}[h!]
	...
	\hfill
	\subfloat[Matched door]{\includegraphics[width=0.3\textwidth]{point_clustering_multiple_spread}\label{fig:PolyDPMatch41}}
	\caption{Polygon matching template and matches.}
	\label{fig:PolyDPMatch41}
\end{figure}
